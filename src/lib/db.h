...
private:
  explicit DB(int e_, int b_, int q_) : exchange(e_), base(b_), quote(q_)
  {
    ...
    if (sqlite3_open(dbFpath.data(), &db))
  ...
  static void insert(const FunctionCallbackInfo<Value> &args)
  {
    ...
    long time = args[4]->IsUndefined() ? 0 : args[4]->Uint32Value();
    sqlite3_exec(db,
                 string((rm || id != "NULL" || time) ? string("DELETE FROM ").append(table).append(id != "NULL" ? string(" WHERE id = ").append(id).append(";") : (time ? string(" WHERE time < ").append(to_string(time)).append(";") : ";")) : "").append((args[1]->IsUndefined() || row.IsEmpty()) ? "" : string("INSERT INTO ").append(table).append(" (id,json) VALUES(").append(id).append(",'").append(*String::Utf8Value(row.ToLocalChecked())).append("');")).data(),
                 NULL, NULL, &zErrMsg);
    if (zErrMsg)
      printf("sqlite error: %s\n", zErrMsg);
    sqlite3_free(zErrMsg);
